...
public class Table
{
    ...
    // Shared Forks (Objects to lock)
    public object[] Forks { get; } = new object[4];

    public Table(string name, string p1, string p2, string p3, string p4)
    {
        Name = name;
        for (int i = 0; i < 4; i++) Forks[i] = new object();

        // Assign Philosophers and their forks (Left/Right)
        ...
        Students[3] = new Student(p4, Forks[3], Forks[0]); // Circular
    }
}